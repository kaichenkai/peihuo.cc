...
const vue = new Vue()
const redirectDict = {
    '/indent': '/branchStoreLogin',
    '/branchStoreLogin': '/branchStoreLogin'
}
// 401 未登录 403 无权限
export default {
    ...
    repInterceptor: [function(response) {
        ...
        try {
            ...
            } else if (response.data.error_code === 401 || response.data.error_code === 403) {
                vue.$notify.error({
                    title: '错误',
                    message: response.data.error_text
                })
                router.replace({
                    path: redirectDict[router.history.current.path] || '/login',
                    query: {
                        redirect: router.history.current.fullPath
                    }
                })
                return Promise.reject(response.data.error_text)
            } else {
                return Promise.reject(response.data.error_text)
            }
        } catch (e) {
